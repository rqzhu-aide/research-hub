...
rather than quadratic. If a current context has size S and a branch is run n
times, the copied-input growth becomes O(nS) instead of O(n²S).
...
Phase 5 may run only when the selected branch has complete, up-to-date Phase 3
and Phase 4 heads matching the exact active method definition. Global Phase 1
and Phase 2 requirements must also be satisfied.
...
There is no user approval action. A finalized run with a valid scientific
outcome (Complete or Partial) is complete and ready. A head is up to date when
its frozen method identity matches the active method. Phase 5 requires
complete, up-to-date P3 and P4 heads. If a method has been revised, P3 and P4
must be rerun before P5 can run. If P3 or P4 has no current head, P5 cannot
run. Legacy `approved_run` fields remain in project state for backward
compatibility but do not affect context selection or Phase 5 readiness.

### 3.4 Method revisions

Every method-bound head is tied to (stable method ID, method version, method
definition SHA-256).
...
- proposed complete head and head digest;
- preparation time.
...
- both heads match the active method (up to date);
...
Routine P5 readiness requires no user action beyond launching the run. There is
no approval step.
...
There is no approval step. A completed run becomes current automatically when
finalized. Legacy `approved_run` fields remain in project state but are not
displayed in the UI and do not affect workflow. The UI must not claim that
archived-run selection already exists.
...
- routine readiness requires no user action beyond launching the run.
